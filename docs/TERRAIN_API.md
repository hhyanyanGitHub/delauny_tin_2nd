...
本文说明 dterrain 0.85 的 `dt_terrain_api.h` 和 `dt_task_api.h`。原
...
if (status == DT_OK) {
    ...
    // 可在缓存仍存活时维护；库会与后台瓦片读写同步。
    dt_grid_view_cache_compact(cache, &compact);
    dt_grid_view_cache_destroy(cache);
}
```
...
已存记录数、可确定回收字节、目录命中次数、成功写入数、停写/容量跳过数和源指纹。
可回收值只计算被新版本覆盖的历史记录，不主动读取负载查坏块。`DISK_CACHE_HIT` 结果标志表示至少
...
包满、只读或写失败不会使视口任务失败。Windows 上同一路径跨进程只允许一个可写缓存；
第二写者返回 `DT_E_IO`。`dt_grid_view_cache_compact()` 原子保留最后有效记录并报告回收量，
可与同句柄后台读任务安全协调；只读包返回 `DT_E_UNSUPPORTED`，纯内存缓存返回
`DT_E_NOT_FOUND`。关闭所有句柄后也可安全删除并重建。
完整布局见 [DGTILE_FORMAT.md](DGTILE_FORMAT.md)。

### 跨层级渐进视口（v0.37）

交互程序希望先得到粗图、再连续替换为精细图时，使用同一缓存句柄提交渐进任务：

```cpp
dt_grid_progressive_view_options progressive{};
progressive.struct_size = sizeof(progressive);
progressive.initial_lod_multiplier = 4; // 0 也表示默认 4
progressive.maximum_frame_count = 3;    // 0 也表示默认 3

dt_task_handle task = nullptr;
dt_grid_read_view_progressive_async(cache, &request, &progressive, &task);
...
for (;;) {
    ...
    if (available) {
        ...
        if (dt_task_get_grid_view_frame(task, sequence, &frame) == DT_OK) {
    ...
    // 也可读取 dt_task_info，处理 FAILED/CANCELLED。
}
dt_task_destroy(task);
```

`initial_lod_multiplier` 必须为不大于 1024 的 2 次幂；最大帧数范围为 1～8，最终目标
LOD 始终发布。默认目标为 `s1` 时序列是 `s4/s2/s1`；目标为 `s8` 时是
`s32/s16/s8`。若帧数不足，中间尺度会截断，但粗首帧和目标帧保留。帧结构中的
`sequence` 从 1 递增，getter 返回第一个大于 `after_sequence` 的帧；没有新帧时返回
`DT_E_NOT_FOUND`。`values` 在任务销毁前有效，调用方可安全保留旧帧指针。

渐进任务的结果种类仍为 `DT_TASK_RESULT_GRID_VIEW`。任务成功后可继续调用
`dt_task_get_grid_view_result()` 取得最终帧，便于与旧代码共用提交后的完成处理。
预取/块校验只执行一次并把摘要传播到后续帧；取消不会发布半帧。显示输出建议保持在
屏幕级尺寸，因为任务会为指针稳定性保留已发布帧。
全部保留帧的 double 像素总量上限为 512 MiB，超出返回 `DT_E_LIMIT_EXCEEDED`。
...
`dt_polygon_rings` 使用一个外环和零个或多个孔洞环。`ring_offsets` 长度为
`ring_count+1`，从 0 开始并以 `point_count` 结束；环隐式闭合，输入不得重复首点。
Z 被忽略。`dt_tin_clip_polygon_exact()` 和 `dt_cdt_clip_polygon_exact()` 先按多边形
包围盒查询面索引，再逐候选三角形求解析交集，因此无需遍历范围外的大网面。

返回的 `dt_surface_clip_result_view` 是三层借用视图：

- `pieces`：每个源三角形产生的交集片段及其环范围；
- `rings`：外环/孔洞标记和扁平点数组范围；
- `points`：边界 XY 及源三角面线性插值得到的 Z；
- `exact_plan_area`：所有片段外环减孔洞后的投影面积和。

视图在 `dt_surface_clip_result_destroy()` 前有效。结果保持原三角面分片，不把带孔洞多边形
强行三角扇化；需要 GPU 三角形时，调用方应使用支持孔洞的多边形三角化器。

```cpp
uint64_t offsets[] = {0, outer_count, outer_count + hole_count};
...
`dt_grid_register_surface()` 估计施加到移动面的 2.5D 平移 `(dx,dy,dz)`。两 GRID 的
CRS 字符串必须一致，函数不会隐式重投影。XY 使用由粗到细的确定性搜索；每个候选共享
同一组 Halton 样本。设置 `DT_SURFACE_REGISTRATION_ESTIMATE_Z_OFFSET` 后，候选的 DZ
取共同有效样本高差均值。结果报告配准前后 RMSE、覆盖率、有效样本数、候选数和迭代数。
当前不估计旋转、尺度或非刚性形变。

约定是把结果施加到移动面坐标/高程：`(x+dx,y+dy,z+dz)`。因此在参考位置 `(x,y)`
求已配准移动面时，从原移动 GRID 的 `(x-dx,y-dy)` 采样并加 `dz`。
`dt_grid_apply_registration()` 按此约定生成与参考 GRID 行列数及完整六参数仿射相同的新
GRID，范围外或 NoData 支撑写为 NaN NoData。

`dt_grid_compare_surface_adaptive()` 以相同确定性序列比较两面，可选择应用给定配准量。
结果包括误差极值、均值、MAE、RMSE、最大绝对误差、覆盖率及 RMSE 标准误差。采样至少
达到 `minimum_samples` 后，每 256 个共同有效点检查一次停止条件；标准误差不高于
`target_rmse_standard_error` 时设置 `DT_SURFACE_ERROR_CONVERGED` 并提前结束，否则直到
`maximum_samples`。该统计适合研究验证；空间自相关明显时，置信解释应结合项目采样规范。
