...
def build_developer_manual():
    ...
    add_document_control(
        m,
        "架构、构建部署、稳定 C ABI、DGRIDB 二进制映射/金字塔/视口块校验缓存、DGTILE 稀疏持久显示瓦片、世界视口 GRID 裁剪与异步 LOD、GRID 窗口概览、GRID 多边形裁剪/掩膜、GRID 显式重采样、双 GRID 精确挖填方、局部与全幅坡度/坡向/阴影分析、约束 Delaunay、GRID/等高线转换、GDAL 格式交换、异步任务、12 个兼容接口、文件格式、性能、线程安全、故障排查。",
        [
            "首次集成：重点阅读第 3、4、7 章。",
    ...
    for text in (
        ...
        "独立 CDT 句柄、断裂线、外边界、孔洞、局部新增、交叉节点化，以及 DCDT/DCDTB 往返。",
    ...
    m.callout("适用边界", "GRID→TIN 和等高线→TIN 都重建普通 Delaunay；严格折线、边界和孔洞应使用独立 dt_cdt_handle。v0.70 新增无交叉断裂线局部拓扑路径；边界、改形、删除、自动节点化和批量事务仍完整构建原子候选网。GUI 多边形土方针对水平基准面并使用固定预算数值积分；双 GRID 接口要求节点对齐，错位表面可先显式重采样。所有土方结果均需结合项目规范复核后用于工程结算。", "gold")
...
dt_release_query_result(result);""")
    ...
    m.table(
        ...
        [
            ...
            ("dt_grid_save/load_binary", "DGRIDB 1", "大 GRID 映射加载、持久概览和原子保存。"),
            ("dt_contours_save/load_text", "DCONTOUR 1", "等高折线文本往返。"),
            ("dt_cdt_save/load_text", "DCDT 1", "散点、约束和 CRS 文本往返。"),
            ("dt_cdt_save/load_binary", "DCDTB 1", "带范围目录、偏移和全载荷校验的约束网二进制。"),
        ],
        [3200, 1800, 4360],
        [WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.CENTER, WD_ALIGN_PARAGRAPH.LEFT],
    )
...
dt_cdt_destroy(cdt);""")
    ...
    m.callout("性能与交叉", "拒绝交叉模式下新增普通断裂线复制原子候选后直接局部插入 CGAL 拓扑，不从全部基础点重建；边界、改形、删除和自动节点化仍完整重建。SPLIT_COMPATIBLE_Z 用 R-tree 筛选相交段，并仅在两线插值 Z 满足显式容差时写入共享交点；共线重叠始终拒绝。dt_cdt_get_edit_metrics() 可核实实际路径。", "gold")
    m.code("""dt_cdt_options options{};
options.struct_size = sizeof(options);
options.crossing_policy = DT_CDT_CROSSING_SPLIT_COMPATIBLE_Z;
options.crossing_z_tolerance = 0.001;
dt_cdt_create(&options, &cdt);
...
if (s == DT_OK) {
    ...
    m.h2("4.16 DGRIDB 映射、金字塔与块校验")
    m.para("dt_grid_save_binary() 保存 DGRIDB 1，dt_grid_load_binary() 加载后仍返回普通 dt_grid_handle。文件包含固定 64 KiB 校验头、完整六参数仿射、NoData、UTF-8 CRS、有效值统计、最多 512×512 的兼容全幅概览、从 1/2 分辨率逐级生成的平均金字塔、4 MiB 原始节点块校验表和行优先 double 节点。Windows 分别用 PAGE_WRITECOPY / FILE_MAP_COPY 映射原始节点和各金字塔层；非 Windows 构建回退为读入内存。")
    m.code("""dt_grid_save_binary(grid, "terrain.dgridb");

dt_grid_handle mapped = nullptr;
if (dt_grid_load_binary("terrain.dgridb", &mapped) == DT_OK) {
    dt_grid_info info{};
    info.struct_size = sizeof(info);
    dt_grid_get_info(mapped, &info);
    bool is_mapped =
        (info.flags & DT_GRID_STORAGE_MEMORY_MAPPED) != 0;
    // mapped 可继续传给窗口、概览、分析、等高线和转换 API
    dt_grid_destroy(mapped);
}""")
    m.para("DT_GRID_STORAGE_MEMORY_MAPPED、DT_GRID_HAS_PERSISTENT_OVERVIEW、DT_GRID_HAS_PYRAMID 和 DT_GRID_HAS_BLOCK_CHECKSUMS 是 dt_grid_info.flags 的输出能力位，不可传入 dt_grid_create_options。平均概览显式设置 DT_GRID_OVERVIEW_USE_PYRAMID 后，库选择仍不小于输出的最低分辨率层，并以 USED_PYRAMID 标识显示近似；未设置时保持逐源节点精确分箱。dt_grid_prefetch_window() 提示操作系统预取原始映射页面。")
    m.code("""options.flags |= DT_GRID_OVERVIEW_USE_PYRAMID;
dt_grid_read_overview(mapped, &options, 512, 256, pixels, 0, &result);
bool fast_lod = (result.flags & DT_GRID_OVERVIEW_USED_PYRAMID) != 0;

// 归档或传输后主动完整扫描；普通打开不会扫描所有节点
dt_status verified = dt_grid_verify_binary_file("terrain.dgridb");""")
    m.callout("编辑失效规则", "任意窗口写入都会清除当前句柄的持久概览、金字塔和块校验能力，避免继续使用陈旧派生数据；写时复制页面仍不会回写源文件。下一次二进制保存统一重建这些段。", "blue")
...
int32_t completed = 0;
if (s == DT_OK &&
    dt_task_wait(task, UINT32_MAX, &completed) == DT_OK && completed) {
...
dt_task_destroy(task);""")
    ...
    m.h2("4.18 DGRIDB 视口块校验与缓存")
    m.para("v0.31 可只校验当前准备读取的 DGRIDB 原始节点窗口。dt_grid_verify_window() 把二维窗口逐行映射到既有 4 MiB 连续校验块，去重后读取完整相交块；dt_grid_verify_result 固定为 64 字节，分别报告块总数、本次冷校验数、缓存命中数和实际检查字节数。后者可能大于窗口本身，因为校验粒度是完整块。")
    m.code("""dt_grid_verify_result vr{};
vr.struct_size = sizeof(vr);
dt_status s = dt_grid_verify_window(
    mapped, column, row, width, height, &vr);

// 也可提交到统一任务运行时
dt_task_handle task = nullptr;
dt_grid_verify_window_async(
    mapped, column, row, width, height, &task);
int32_t completed = 0;
dt_task_wait(task, UINT32_MAX, &completed);
dt_grid_verify_result async_vr{};
async_vr.struct_size = sizeof(async_vr);
dt_task_get_grid_verification_result(task, &async_vr);
dt_task_destroy(task);""")
    m.para("异步结果类型为 DT_TASK_RESULT_GRID_VERIFICATION。任务共享持有源 GRID，按块报告进度，在块边界响应协作取消；失败或取消不发布部分统计。无校验扩展的 v0.28 文件和普通内存 GRID 返回 DT_E_UNSUPPORTED。")
    m.code("""// 在概览读取前自动验证同一源窗口
...
dt_grid_read_overview_async(grid, &options, 512, 512, &task);""")
    m.callout("缓存与并发", "通过块和失败块状态都保存在当前已加载 GRID 句柄中并受互斥保护；重复窗口通常直接命中缓存。并发只读校验安全，但不得同时写入同一 GRID。任意节点写入会清除旧校验能力和缓存，重新保存 DGRIDB 时重建。", "blue")
    m.callout("保护边界", "VERIFY_SOURCE_BLOCKS 验证的是请求窗口对应的原始 double 节点块，即使后续概览直接来自持久全幅缓存或金字塔。它不校验概览/金字塔载荷，也不能替代归档或复制后的 dt_grid_verify_binary_file() 全量审计；高保障分发还应配合外部 SHA-256。", "gold")
    m.callout("本机基准", "8192×4096、343.19 MiB DGRIDB：12 个相交块（48 MiB）冷校验约 58.56 ms，同一窗口缓存复查约 6 μs，完整原始节点扫描约 384.06 ms；冷窗口路径约快 6.56×。", "gold")

    m.h2("4.19 DGRIDB 流式金字塔保存")
    m.para("v0.32 保持 dt_grid_save_binary() 的 C ABI 和 DGRIDB 1 文件布局不变，但不再把全部 2× 平均金字塔层同时构造在内存。保存先计算每层尺寸、比例尺和 64 KiB 对齐偏移；第一层从源 GRID 读取，后续层每次从临时文件中的上一层读取两行，生成一行后直接写入当前层。峰值临时内存由约三分之一原始节点量降为约两条源行加一条输出行。")
    m.code("""dt_status s = dt_grid_save_binary(grid, "terrain.dgridb");
// 公开调用方式与旧版本相同；保存后的文件仍包含：
// 512×512 以内全幅概览、2× 多级金字塔、4 MiB 块校验和原始节点。""")
    m.callout("同路径覆盖", "若 grid 本身映射自目标 DGRIDB，库会在临时文件完成后实体化当前 COW 修改并释放旧映射，原子替换成功后重新映射新原始节点和各级金字塔；句柄继续有效。任何失败都删除临时文件，原目标不被半成品覆盖。", "blue")
    m.callout("普通内存句柄", "普通内存 GRID 保存后不让当前句柄映射刚写出的金字塔，以免活动 Windows section 阻碍其他句柄覆盖同一目标。文件内金字塔完整存在；需要立即使用 LOD 时，销毁普通内存句柄并重新加载刚保存的 DGRIDB。", "gold")
    m.callout("本机基准", "8192×4096、343.19 MiB：旧完整金字塔临时数组理论值 85.00 MiB，v0.32 逐行缓冲约 0.078 MiB，保存约 0.576 s；4096×2048 由 21.00 MiB 降至 0.039 MiB，保存约 0.158 s。", "gold")

    m.h2("4.20 统一异步视口流水线")
    m.para("v0.33 的 dt_grid_read_view_async() 把世界坐标视口映射、操作系统预取、可选 DGRIDB 块校验和金字塔/精确 LOD 合并为一个任务。调用方只提交 dt_grid_view_request_options；任务成功后以 DT_TASK_RESULT_GRID_VIEW 标识，dt_task_get_grid_view_result() 同时返回实际源窗口、任务自有只读像素、LOD 统计和可选校验摘要。旧窗口、预取、校验和异步概览接口继续可独立使用。")
    m.code("""dt_grid_view_request_options request{};
...
if (dt_task_get_grid_view_result(task, &result) == DT_OK) {
    upload_preview(result.values, result.width,
                   result.height, result.row_stride);
}
dt_task_destroy(task);""")
    m.callout("进度与失败原子性", "请求校验时，校验占进度 0～30%，LOD 占 30～100%；未请求校验时 LOD 占全部进度。世界视口无交集、校验损坏、非法尺寸和协作取消均不会发布部分像素，getter 只接受完整成功结果。", "blue")
    m.callout("选项与能力", "STRICT_NODATA 和 USE_PYRAMID 对应显示采样策略；PREFETCH_SOURCE 是最佳努力提示；VERIFY_SOURCE_BLOCKS 要求句柄声明 DT_GRID_HAS_BLOCK_CHECKSUMS，否则任务以 DT_E_UNSUPPORTED 失败。请求选项固定 96 字节，结果中的 values 只在 task 销毁前有效。", "gold")
    m.callout("GUI 集成", "演示程序仍用同步窗口查询建立轻量去重键，但后台统一任务会重新计算并返回实际窗口，发布前再次核对。上一帧保留、25 ms 轮询、最多三个退休任务、过期取消和同键失败抑制保持不变。", "blue")
    m.callout("本机基准", "4096×4096、1024×768 源窗口到 512×512：同步局部 LOD 约 2.28 ms，统一任务提交 54.3 μs、后台完成 2.65 ms，与同步结果抽样校验和误差为 0。", "gold")

    m.h2("4.21 二维空间瓦片缓存与跨请求合并")
    m.para("v0.34 的 dt_grid_view_cache_handle 在统一视口任务之上增加可复用显示缓存。目录键包含 GRID generation、2 次幂 LOD、二维瓦片坐标、概览方法和影响数值的 NoData/金字塔策略。dt_grid_read_view_cached_async() 仍返回 DT_TASK_RESULT_GRID_VIEW，并继续用 dt_task_get_grid_view_result() 取得任务自有像素。缓存路径明确设置 DT_GRID_OVERVIEW_USED_TILE_CACHE，不替代精确分析。")
    m.code("""dt_grid_view_cache_options cache_options{};
cache_options.struct_size = sizeof(cache_options);
cache_options.tile_width = cache_options.tile_height = 128;
...
view.struct_size = sizeof(view);
dt_task_get_grid_view_result(task, &view);
// view.lod_scale / tile_count / reused_tile_count
dt_task_destroy(task);

dt_grid_view_cache_statistics stats{};
stats.struct_size = sizeof(stats);
dt_grid_view_cache_get_statistics(cache, &stats);
dt_grid_view_cache_destroy(cache);""")
    m.callout("调度与合并", "每个视口按瓦片中心到视口中心的距离排序；共享优先队列允许新视口的中心瓦片越过旧视口边缘瓦片。相同键若已排队或加载，后来的任务只增加需求计数并等待同一结果，不重复聚合。单个订阅者取消不会中断仍被其他请求需要的瓦片。", "blue")
    m.callout("容量与生命周期", "默认 128×128、128 MiB、4096 项、最多 8 个自动生产线程；完成项同时按字节和数量 LRU 淘汰。活动任务以 shared_ptr 借用瓦片和缓存实现，所以目录项或公开缓存句柄删除后，已提交任务仍安全。GRID generation 改变后自动使用新键。", "gold")
    m.callout("结果语义", "缓存 LOD 是交互显示近似。USED_TILE_CACHE 表示该路径，CACHE_HIT 和 CACHE_COALESCED 分别表示命中完成项和加入在途项；overview 统计只覆盖返回像素，不设置 EXACT_SOURCE_STATISTICS。体积、坡度、等高线、转换和导出继续使用完整 GRID。", "gold")
    m.callout("本机基准", "8192×4096、1024×768→512×512：冷缓存约 6.64 ms，热缓存约 3.30 ms，48/48 瓦片复用、约 2.01×；平移 128 个源节点后复用 42 个瓦片并约 5.00 ms。", "blue")

    m.h2("4.22 DGTILE 跨会话显示缓存")
    m.para("v0.35 的 dt_grid_view_cache_create_persistent() 在原内存目录下增加 DGTILE 1 稀疏旁车。打开时只扫描 4 KiB 文件头和 128 字节记录头，像素负载在瓦片命中后才读取并校验；没有命中的瓦片仍从源 GRID 生成并按需追加。后续任务接口、结果生命周期、中心优先调度、在途合并和内存 LRU 与 v0.34 完全相同。")
    m.code("""dt_grid_view_disk_cache_options disk{};
disk.struct_size = sizeof(disk);
disk.flags = DT_GRID_VIEW_DISK_CACHE_RESET_STALE |
             DT_GRID_VIEW_DISK_CACHE_RESET_CORRUPTED;
disk.utf8_file_name = "terrain.dgridb.dgtile";
disk.source_revision = application_revision;
disk.maximum_file_bytes = 2ULL * 1024 * 1024 * 1024;

dt_grid_view_cache_handle cache = nullptr;
dt_status s = dt_grid_view_cache_create_persistent(
    grid, &cache_options, &disk, &cache);

dt_grid_view_disk_cache_statistics ds{};
ds.struct_size = sizeof(ds);
dt_grid_view_cache_get_disk_statistics(cache, &ds);""")
    m.callout("失效与恢复", "源指纹包含应用 revision、GRID 尺寸、仿射、CRS、NoData 和最多 64×64 个确定性样本。内存 GRID 任意语义修改仍应更新 revision。RESET_STALE 原子重建不匹配包；RESET_CORRUPTED 重建坏头/目录，并在延迟发现坏负载时重算该瓦片。只有实际成功读取磁盘负载的任务才设置 DISK_CACHE_HIT。", "blue")
    m.callout("降级边界", "默认容量 2 GiB，包满、只读或写失败只停止追加，显示继续从内存/源数据完成。dt_grid_view_cache_clear() 不删除旁车；DGTILE 不是权威地形数据，关闭句柄后可直接删除。", "gold")
    m.callout("并发约束", "同一规范化路径只允许一个可写缓存；Windows 还通过命名互斥量拒绝另一进程的第二写者。只读句柄不获取写者互斥量。GUI 自动为 DGRIDB 使用 .dgridb.dgtile，失败时回退为纯内存目录。", "red")
    m.callout("本机基准", "8192×4096、128×128 瓦片、1024×768→512×512：首次填充约 14.47 ms；重建缓存句柄后从磁盘复用约 8.78 ms，48/48 瓦片命中，抽样误差为 0。", "blue")

    m.h2("4.23 DGTILE 在线压缩与空间回收")
    m.para("v0.36 的 dt_grid_view_cache_compact() 在缓存句柄存活时，把当前旁车重写为每个键最后一条有效记录。库按稳定键序逐块读取并复核校验，只在新文件完整刷新后原子替换旧文件。后台瓦片读取持有共享文件锁，追加和压缩持有独占文件锁；压缩提交后加载任务会按键重新取得最新偏移，因此不会读取替换前的陈旧位置。")
    m.code("""dt_grid_view_cache_compact_result compact{};
compact.struct_size = sizeof(compact);
dt_status s = dt_grid_view_cache_compact(cache, &compact);
// input/output_file_bytes、reclaimed_bytes
// dropped_duplicate_record_count、dropped_corrupt_tile_count""")
...
dt_task_destroy(task);""")
    ...
    m.h2("4.25 Direct3D 11 分块三维集成")
    m.para("v0.40 的三维实现位于 GUI 集成层，不改变 DLL C ABI。terrain_gpu.cpp 把完整 TIN 按 16,384 面切分为不可变顶点缓冲，每个顶点携带归一化位置、高程比例、面法向和全局三角形 ID；块包围球用于相机视锥粗裁剪，GPU 深度缓冲替代软件画家排序。硬件设备失败后尝试 WARP，仍失败则隐藏 D3D 子窗口并回退原 GDI 路径。")
...
1 3 4""")
    ...
    m.h2("6.5 DGRIDB 二进制规则网格")
    m.para("DGRIDB 1 是本机大 GRID 工作格式。固定头记录源统计、兼容概览、金字塔目录、校验表和原始节点偏移；各映射数据段按 64 KiB 对齐。v0.29 扩展放在旧概览与原始节点之间，旧加载器依靠原始节点偏移跳过扩展，新加载器也接受无扩展 v0.28 布局。头校验不覆盖载荷；内置块校验只覆盖原始节点，若需覆盖整个文件或抵抗恶意篡改仍应配 SHA-256。")
    m.h2("6.6 DCONTOUR 等高线文本")
    m.para("DCONTOUR 1 由 LINES 计数和若干 LINE 记录组成。每条 LINE 保存等高值、闭合标志、点数及 XYZ 顶点。逐线视图中的点数组只在等高线句柄释放前有效。")
    m.h2("6.7 GeoTIFF、COG 与 GeoPackage")
    ...
    m.h2("6.8 DCDT / DCDTB 约束网")
    m.para("DCDT 1 保存 CRS、基础 XYZ 点和约束记录。每条约束包含稳定 ID、类型、闭合标志、点数和 XYZ；加载时完整重建 CDT 并重新标记域，失败不会覆盖原对象。DCDTB 1 增加固定 64 字节约束目录、XY 包围盒、点载荷绝对偏移及 FNV-1a 全载荷校验，适合大约束网。")
    m.para("dt_cdt_query_binary_index() 只扫描目录即可按窗口列出约束；dt_cdt_read_binary_constraint() 按稳定 ID 直接读取一条折线，均不要求创建或构建 CDT。完整加载会先验证校验值，再原子替换目标句柄。")
...
dt_get_last_error(message, sizeof(message), &required);""")
    ...
    m.table(
        ...
        [
            ...
            ("dt_grid_view_cache_handle", "dt_grid_view_cache_create", "dt_grid_view_cache_destroy"),
    ...
    for text in (
        ...
        "大 GRID 本机工作优先 DGRIDB；交互显示使用异步概览、金字塔和 VERIFY_SOURCE_BLOCKS 按需校验，分析计算保持精确原始节点路径。",
        "DGRIDB 视口按需校验只覆盖已访问的原始节点块；复制传输或归档后仍调用 dt_grid_verify_binary_file() 完整扫描。",
...
def build_gui_manual():
    ...
    add_document_control(
        m,
        "便携运行、五分钟上手、控件与菜单、XYZ 导入、DGRIDB 二进制映射 GRID、DGTILE 自动旁车缓存、TIN/GRID/等高线/CDT 图层、非阻塞大 GRID 视口 LOD、任意多边形 GRID 裁剪/掩膜、错位 GRID 最近邻/双线性对齐、现状/设计双 GRID 土方、全幅坡度/坡向/阴影专题图、局部坡面、任意剖面、多边形面积/土方、2D/3D 浏览、查询编辑和数据保存。",
        [
            "第一次体验：直接完成第 2 章的五分钟教程。",
            "已有 XYZ 数据：重点阅读第 5 章。",
            "三维地形展示：重点阅读第 4 章。",
            "大 GRID 视口自适应 LOD：重点阅读第 4.4 节。",
            "DGRIDB 导入导出：重点阅读第 4.4 节和第 8.3 节。",
            "任意剖面分析：重点阅读第 6.3 节。",
            "面积与土方量测：重点阅读第 6.4 节。",
    ...
    m.callout("0.70 功能边界", "CDT 自动节点化只处理可表达为单一 2.5D 高程的交点；两线插值 Z 超过容差或共线重叠时拒绝。无交叉断裂线采用局部拓扑插入，但为了原子提交仍复制候选状态；边界和既有约束改形继续完整重建。TIN、GRID、等高线重投影与 Direct3D 三维显示保持 v0.55/v0.40 行为。", "gold")
    m.h2("1.1 运行文件")
    m.table(
        ["文件", "作用"],
        [
    ...
    steps = [
        ...
        "单击“切换3D”，左键拖动环视、滚轮拉近，用 WASD 贴地漫游；Ctrl+左键拾取一个面，按 F 切换贴地，再观察垂直夸张。",
        "单击“切换2D”返回二维编辑视图。",
        ...
        "打开“数据交换→打开约束网 DCDT / DCDTB”，选择 sample_constraints.dcdt，观察孔洞和断裂线。",
    ...
    m.h2("4.2 三维查看与漫游")
    m.para("单击“切换3D”后，完整 TIN 由 Direct3D 11 按块透视显示。状态栏给出可见块/总块和本帧面/总面；设备不可用时自动尝试 WARP，再失败才进入软件回退。三维只负责查看，选择查询、插入、删除或框选放大时会自动返回二维，防止把透视屏幕坐标误当作地形 XY。")
    m.table(
        ["输入", "三维操作"],
        [
            ("左键拖动", "绕观察目标环视。"),
    ...
    m.callout("大坐标与拾取", "三维模型先减去数据中心并按 XY 范围归一化，再使用双精度相机计算，可直接查看局部坐标或大数值投影坐标。Ctrl+左键只回读一个整数 ID 像素；贴地高度由 DLL 精确 TIN 点定位和插值获得。", "blue")
    ...
    m.callout("DGRIDB 金字塔与校验参考", "8192×4096、343.19 MiB 文件：12 个视口块（48 MiB）冷校验约 58.56 ms、缓存复查约 6 μs、完整扫描约 384.06 ms；4096×2048 局部窗口到 512×256 的金字塔路径约 1.84 ms。编辑后旧金字塔/校验失效，保存时自动重建。", "blue")
...
500010.0;3200010.0;106.80 # ridge""")
    ...
    m.h2("8.3 DGRIDB、DGRID 与 DCONTOUR")
    m.para("通过“数据交换”菜单可导入或导出 DGRIDB 二进制映射 GRID、DGRID 规则高程节点文本和 DCONTOUR 等高线文本。菜单还提供“验证 DGRIDB 数据块”：完整扫描每个 4 MiB 原始节点块，适用于复制传输或归档后核验；普通打开不自动扫描。导入这些图层不会删除现有 TIN，因此适合叠加检查。GRID 支持完整六参数仿射变换。")
    ...
    m.h2("8.5 DCDT 约束网")
    m.para("选择“数据交换→打开约束网 DCDT / DCDTB”可加载基础点、外边界、孔洞、断裂线和 CRS。域内网为紫色，外边界青色，孔洞洋红，断裂线橙色。sample_constraints.dcdt 可直接验证；保存时按扩展名选择 DCDT 可读文本或带范围目录与校验的 DCDTB 二进制，默认使用 .dcdtb。")
    ...
    for step in (
        ...
        "若移动造成插值 Z 超容差、共线重叠等非法拓扑，原约束保持不变，选中状态保留，可换一个目标位置重试或按 Esc 取消。",
        ...
        m.number(step)
    m.callout("高程与性能", "草图和移动目标的 Z 优先由 CDT/TIN 表面插值。GUI 默认以 0.001 当前高程单位容差自动节点化交叉；无交叉普通断裂线走局部 CGAL 拓扑插入，边界、移动、删除和批量事务仍完整构建原子候选。移动/删除顶点还请求完整差异用于影响显示。查询、插入和删除工具栏按钮仍只操作普通 TIN。", "gold")
    m.h2("8.7 往返验证")
    for step in (
        "保存当前网格。",
        "记录状态栏的顶点数和三角形数。",
    ...
    m.h1("9 大数据量演示建议")
    m.para("DLL 对当前视口执行精确范围查询。为了避免 GDI 阻塞，二维 TIN/CDT 线框预算各约 45,000 面；三维 Direct3D 主路径按 16,384 面分块并提交视锥内全部块，不设 18,000 面抽样上限，只有软件回退仍使用该预算。任意规模 GRID 先裁剪当前世界视口的源节点窗口，再生成不超过 512×512 的局部概览；等高线绘制预算约 20 万顶点。显示策略不会删减 DLL 数据或文件导出内容。")
    m.h2("9.1 百万级点操作顺序")
    for text in (
        ...
        "三维优先使用 GPU 分块和视锥裁剪；千万点连续漫游仍应控制同时载入范围，后续版本将增加块 LOD、后台上传和显存 LRU。",
    ...
    faq = [
        ...
        ("没有出现‘（磁盘）’", "仅 DGRIDB 自动启用 DGTILE，且必须重新打开后命中已浏览瓦片；DGRID、GeoTIFF 和派生专题默认只用内存缓存。旁车创建失败时程序会自动回退。"),
        ("DGTILE 文件越来越大", "追加式稀疏缓存默认上限 2 GiB。选择‘数据交换→压缩当前 DGTILE 显示缓存’可在线回收重复/坏记录；也可关闭程序后删除 .dgtile，后续按需重建。"),
        ("DGTILE 压缩失败", "确认当前 DGRIDB 已成功打开、目录可写，且没有另一个进程持有同一旁车的写句柄。失败不会破坏旧包，显示会继续使用内存或源 GRID。"),
        ("显示很稀疏", "全图抽样属于渲染策略；框选放大局部后会显示更完整细节。"),
        ("左键没有查询", "检查当前是否处于框选、插入、删除或三维视图。"),
        ("三维地形太平", "单击“高程×n”或按 + 提高垂直夸张。"),
        ("三维视点迷失", "按 Home 或单击“全图”恢复适屏相机。"),
        ("状态栏显示软件3D回退", "更新显卡驱动并确认系统支持 Direct3D 11；程序已尝试 WARP，回退不影响二维编辑和导出。"),
        ("Ctrl+左键没有拾取", "确认光标位于地形表面且当前不是软件回退；先按 Home 适屏再试。"),
        ...
        ("约束提交失败", "检查点数、连续重合点、交点高差和共线重叠；失败时草图保留，可 Backspace 调整。"),
        ("约束顶点移动失败", "目标位置可能造成交点高差超限、共线重叠或非法边界；原约束不会改变，可再次单击其他位置，或按 Esc 取消。"),
    ...
    for text in (
        ...
        "切换 3D，验证 GPU 块/面统计、环视、滚轮、WASD 贴地、F 切换、Ctrl+左键拾取和垂直夸张，再返回 2D。",
