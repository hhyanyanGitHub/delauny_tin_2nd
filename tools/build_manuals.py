...
def add_document_control(m: Manual, scope, navigation):
    ...
    for item in navigation:
        m.bullet(item)
    m.callout("版本边界", "本手册对应 dterrain 0.21.0：全幅 GRID 专题已接入异步任务，支持进度、Esc/菜单/关闭安全取消和 z-factor/光照参数；同步接口、专题导出、剖面、土方、GDAL 与约束事务保持兼容。双表面土方、局部 CDT 和 GPU LOD 属于后续阶段。", "gold")
...
def build_developer_manual():
    ...
    add_document_control(
        m,
        "架构、构建部署、稳定 C ABI、局部与全幅坡度/坡向/阴影分析、约束 Delaunay、GRID/等高线转换、GDAL 格式交换、异步任务、12 个兼容接口、文件格式、性能、线程安全、故障排查。",
        [
            "首次集成：重点阅读第 3、4、7 章。",
    ...
    m.callout("适用边界", "GRID→TIN 和等高线→TIN 都重建普通 Delaunay；严格折线、边界和孔洞应使用独立 dt_cdt_handle。v0.21 单项约束编辑仍采用候选网全量重建；批量事务把 N 次重建降为 1 次，但尚未达到百万点实时局部 CDT 编辑。GUI 土方量测针对水平基准面并使用固定预算数值积分，不等同于工程结算。", "gold")
...
dt_cdt_destroy(cdt);""")
    ...
    m.callout("性能与交叉", "v0.21 单项约束编辑仍完整重建候选 CDT；批量事务只重建一次。100,000 基础点添加 12 条断裂线的本机对比为逐条 3.259 s、批量 0.528 s，约 6.17 倍。请求 output_effect 还会计算完整域差异。未分段交叉仍须先加入共享顶点。", "gold")
...
if (s == DT_OK) {
    ...
    m.para("dt_grid_derive_terrain() 从源高程 GRID 创建同尺寸、同六参数仿射变换和同 CRS 的标准 dt_grid_handle；源句柄保持不变，输出可继续使用窗口读取、DGRID 保存或 GDAL GeoTIFF/COG 导出。v0.21 新增 dt_grid_derive_terrain_async()，复用统一任务状态、进度和取消接口。输出始终具有 NoData。")
...
dt_task_destroy(task);""")
    ...
    m.callout("异步、内存与量纲", "异步任务按输出行更新进度并在下一行前检查取消；取消状态为 DT_TASK_CANCELLED/DT_E_CANCELLED，不发布半成品。任务持有源 GRID 生命周期，成功结果仍是标准 GRID。时间为 O(width×height)，除源 GRID 外还需一个同尺寸 double 输出 GRID。z_factor 用于高程和平面单位换算；如果 XY 为米而 Z 为毫米，应使用 0.001。", "blue")
...
ctest --test-dir build --output-on-failure""")
    m.para("测试覆盖生命周期、普通 TIN 动态编辑、DTIN/DTMESH/XYZ、TIN/GRID/等高线、等高线反向重建与高程冲突回滚、异步任务，以及 CDT 空网清理、外边界、孔洞、断裂线、交叉失败原子性、TIN→CDT、域内采样、CDT→GRID/等高线、共享顶点保护、批量新增/更新/删除、整批失败回滚和 DCDT 往返。局部坡面测试用解析平面验证 TIN/CDT 的 Z、世界梯度、坡度、坡向和法向，并覆盖孔洞/凸包外拒绝、旋转仿射 GRID 梯度变换、双线性标志和 NoData 拒绝。全幅测试覆盖坡度、坡向、默认光照阴影、CRS 继承、旋转仿射差分、水平坡向 NoData、空洞邻域传播，以及 v0.21 异步专题结果、进度和大 GRID 取消状态。独立剖面数学测试覆盖线性采样、三角形高程、旋转仿射 GRID 双线性插值、NoData 断点和升降/坡度统计；多边形量测测试覆盖凹多边形耳切、大坐标面积稳定性、自相交拒绝、平面解析结果、挖填符号和部分/完全无效域。GDAL 构建还执行 GTiff、COG、GPKG 驱动探测和真实文件往返。")
    m.h2("9.2 集成验收清单")
    for text in (
...
def build_gui_manual():
    m = Manual(
        "dterrain GUI 演示程序\n操作手册与入门教程",
        "GUI 操作入门教程",
        "从 XYZ 散点导入到动态编辑、全幅地形专题图、局部坡面、剖面与土方量测、TIN/GRID/等高线转换、约束 Delaunay、三维漫游与数据保存",
        "首次使用者、演示人员、算法验证与测绘研究人员",
    )
    m.cover("QUICK START GUIDE")
    add_document_control(
        m,
        "便携运行、五分钟上手、控件与菜单、XYZ 导入、TIN/GRID/等高线/CDT 图层、全幅坡度/坡向/阴影专题图、局部坡面、任意剖面、多边形面积/土方、2D/3D 浏览、查询编辑和数据保存。",
        [
    ...
    m.h1("1 程序概览")
    m.para("dterrain_demo.exe 是一个原生 Win32/GDI 演示程序，用于直观验证 dterrain.dll 的批量建网、范围查询、最近点查询、全幅坡度/坡向/阴影专题图、单点坡度/坡向与法向、任意地形剖面、多边形面积/土方量测、动态插入删除、编辑影响显示、文件交换和三维地形漫游。它不依赖 Qt 等 GUI 框架。")
    m.callout("0.21 功能边界", "当前程序已提供二维/三维浏览、可取消且可配置参数的全幅坡度/坡向/阴影专题表达和导出、单击坡度/下坡坡向/单位法向分析、任意直线剖面、多边形面积/水平基准面挖填方与 CSV 导出、TIN/GRID/等高线双向转换、DCDT 打开保存、约束编辑和 CDT 影响区显示；启用 GDAL 后可交换 GeoTIFF/COG GRID、专题 GeoTIFF 与 GeoPackage 等高线。生产级双表面土方和 GPU/LOD 尚未接入。", "gold")
    m.h2("1.1 运行文件")
    m.table(
        ["文件", "作用"],
        [
    ...
    steps = [
        ...
        "先用‘分析→设置专题分析参数’检查 z-factor 与光照角，再生成全幅坡度、坡向和阴影地形；观察进度、固定色带和右上图例，并用‘恢复显示高程 GRID’返回高程着色。",
...
500010.0;3200010.0;106.80 # ridge""")
    ...
    m.h2("6.6 全幅坡度、坡向与阴影专题图")
    for step in (
        "准备一个高程 GRID；若只有 TIN/CDT，直接选择专题命令，程序会自动生成最长边 401 节点的 GRID。",
        "需要换算高程单位或改变阴影方向时，选择‘设置专题分析参数…’，依次输入 z-factor、太阳方位角和高度角。",
    ...
    m.callout("参数约定", "z-factor 默认 1.0 且必须大于 0；XY 为米、Z 为毫米时填写 0.001。太阳方位角以北为 0°顺时针并归一化到 0～360°，高度角范围 -90～90°，默认 315°/45°。", "gold")
    ...
    m.callout("响应与关闭", "专题派生在 DLL 工作线程执行，GUI 以 25 ms 周期刷新进度和分派窗口消息。计算期间其他菜单命令被门控；关闭窗口会先请求取消、等待线程退出并释放任务句柄，再完成退出。", "blue")
    ...
    for step in (
        ...
        m.number(step)
    m.callout("高程与性能", "草图和移动目标的 Z 优先由 CDT/TIN 表面插值。v0.21 单项约束编辑仍完整重建候选 CDT，批量事务只重建一次；移动/删除顶点还请求完整差异用于影响显示。查询、插入和删除工具栏按钮仍只操作普通 TIN。", "gold")
    m.h2("8.7 往返验证")
    for step in (
        "保存当前网格。",
        "记录状态栏的顶点数和三角形数。",
    ...
    m.h1("9 大数据量演示建议")
    m.para("DLL 对当前视口执行精确范围查询。为了避免 GDI 阻塞，二维 TIN/CDT 线框预算各约 45,000 面，三维填充与深度排序预算约 18,000 面，GRID 预览缓存最多 2000 万值并缩放到不超过 512×512，等高线绘制预算约 20 万顶点。显示抽样不会删减 DLL 数据或文件导出内容。")
    m.h2("9.1 百万级点操作顺序")
    for text in (
        ...
        "超过 2000 万节点的 GRID 可以保留和导出，但 GUI 不生成着色缓存或等高线。",
    ...
    faq = [
        ...
        ("专题计算耗时较长", "观察状态栏进度；按 Esc 或取消菜单可安全中止。算法仍需源 GRID 与同尺寸输出 GRID 两份 double 数组。"),
        ("阴影方向或坡度比例不正确", "使用‘设置专题分析参数’核对光照方位/高度和 z-factor；XY 为米、Z 为毫米时 z-factor 应为 0.001。"),
    ...
    for text in (
        ...
        "设置 z-factor 和光照角，依次生成全幅坡度、坡向和阴影专题图，检查进度与固定图例；再启动一次并按 Esc 验证取消，导出 DGRID/专题 GeoTIFF，最后恢复高程显示。",
        "切换 3D，验证环视、滚轮缩放、WASD 漫游和垂直夸张，再返回 2D。",
        "准备原始 XYZ 备份，清空和导入会改变当前内存网格。",
    ):
        m.bullet(text)
    m.callout("推荐演示路线", "导入示例 XYZ → TIN→GRID→等高线 → 全幅坡度/坡向/阴影专题图与导出 → 单击坡度/坡向与法向 → 任意 A—B 剖面与 CSV → 多边形面积/土方量测、改基准面与 CSV → GeoTIFF/COG 与 GeoPackage 往返 → 等高线反向生成 TIN/GRID → 从 TIN 创建 CDT → 批量添加断裂线 → 绘制/移动/安全删除顶点 → CDT→GRID/等高线 → T/G/C/D 显隐 → 3D 漫游。", "blue")
    m.h2("11.1 后续 GUI 升级")
    m.para("当前已交付 2D/3D 浏览，可取消、显示进度且可配置 z-factor/光照角的全幅坡度/坡向/阴影专题图，TIN/CDT/GRID 局部坡面分析、剖面、水平基准土方、格式转换、GDAL 交换及约束编辑。后续重点是专题分块并行与流式输出、局部 CDT 更新、现状面—设计面精确土方、误差控制、可配置图层样式、GPU 分块 LOD 与贴地碰撞。")
