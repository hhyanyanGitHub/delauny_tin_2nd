...
def add_document_control(m: Manual, scope, navigation):
    ...
    for item in navigation:
        m.bullet(item)
    m.callout("版本边界", "本手册对应 dterrain 0.25.0：新增任意简单多边形 GRID 掩膜、紧凑裁剪与反向掩膜，同步/异步接口均支持完整六参数仿射、NoData、并行和取消；显式重采样、解析挖填方、全幅地形专题、剖面、GDAL 与约束事务保持兼容。坐标重投影、流式超大 GRID、网格单元解析边界裁剪、局部 CDT 和 GPU LOD 属于后续阶段。", "gold")
...
def build_developer_manual():
    ...
    add_document_control(
        m,
        "架构、构建部署、稳定 C ABI、GRID 多边形裁剪/掩膜、GRID 显式重采样、双 GRID 精确挖填方、局部与全幅坡度/坡向/阴影分析、约束 Delaunay、GRID/等高线转换、GDAL 格式交换、异步任务、12 个兼容接口、文件格式、性能、线程安全、故障排查。",
        [
            "首次集成：重点阅读第 3、4、7 章。",
...
dt_cdt_destroy(cdt);""")
    ...
    m.callout("性能与交叉", "v0.25 单项约束编辑仍完整重建候选 CDT；批量事务只重建一次。100,000 基础点添加 12 条断裂线的本机对比为逐条 3.259 s、批量 0.528 s，约 6.17 倍。请求 output_effect 还会计算完整域差异。未分段交叉仍须先加入共享顶点。", "gold")
...
def build_gui_manual():
    m = Manual(
        "dterrain GUI 演示程序\n操作手册与入门教程",
        "GUI 操作入门教程",
        "从 XYZ 散点导入到动态编辑、任意多边形 GRID 裁剪、错位设计 GRID 显式对齐、现状/设计双表面土方、全幅地形专题图、局部坡面、剖面与多边形量测、TIN/GRID/等高线转换、约束 Delaunay、三维漫游与数据保存",
        "首次使用者、演示人员、算法验证与测绘研究人员",
    )
    m.cover("QUICK START GUIDE")
    add_document_control(
        m,
        "便携运行、五分钟上手、控件与菜单、XYZ 导入、TIN/GRID/等高线/CDT 图层、任意多边形 GRID 裁剪/掩膜、错位 GRID 最近邻/双线性对齐、现状/设计双 GRID 土方、全幅坡度/坡向/阴影专题图、局部坡面、任意剖面、多边形面积/土方、2D/3D 浏览、查询编辑和数据保存。",
        [
    ...
    m.h1("1 程序概览")
    m.para("dterrain_demo.exe 是一个原生 Win32/GDI 演示程序，用于直观验证 dterrain.dll 的批量建网、范围查询、最近点查询、任意多边形 GRID 裁剪/掩膜、错位设计 GRID 显式对齐、现状/设计双 GRID 挖填方、全幅坡度/坡向/阴影专题图、单点坡度/坡向与法向、任意地形剖面、多边形面积/土方量测、动态插入删除、编辑影响显示、文件交换和三维地形漫游。它不依赖 Qt 等 GUI 框架。")
    m.callout("0.25 功能边界", "当前程序可复用完成的量测多边形，对当前高程 GRID 执行保持范围掩膜、紧凑裁剪或反向掩膜；计算可取消，成功后原子替换图层并清理失效派生结果。错位设计 GRID 对齐、解析挖填方、二维/三维浏览、全幅地形专题、剖面、转换、GDAL 交换和约束编辑继续可用。程序不会隐式重投影；流式超大 GRID、单元解析边界裁剪和 GPU/LOD 尚未接入。", "gold")
    m.h2("1.1 运行文件")
    m.table(
        ["文件", "作用"],
        [
...
500010.0;3200010.0;106.80 # ridge""")
    ...
    for step in (
        ...
        m.number(step)
    m.callout("高程与性能", "草图和移动目标的 Z 优先由 CDT/TIN 表面插值。v0.25 单项约束编辑仍完整重建候选 CDT，批量事务只重建一次；移动/删除顶点还请求完整差异用于影响显示。查询、插入和删除工具栏按钮仍只操作普通 TIN。", "gold")
    m.h2("8.7 往返验证")
    for step in (
        "保存当前网格。",
        "记录状态栏的顶点数和三角形数。",
    ...
    for text in (
        ...
        m.bullet(text)
    m.callout("推荐演示路线", "导入示例 XYZ → TIN→GRID→等高线 → 圈定多边形并作紧凑 GRID 裁剪/掩膜 → 打开错位设计 GRID → 双线性显式对齐 → 双表面挖填方、差值图与 CSV → 全幅坡度/坡向/阴影专题图与导出 → 单击坡度/坡向与法向 → 任意 A—B 剖面与 CSV → 多边形水平基准土方与 CSV → GeoTIFF/COG 与 GeoPackage 往返 → 等高线反向生成 TIN/GRID → 从 TIN 创建 CDT → 批量添加断裂线 → 绘制/移动/安全删除顶点 → CDT→GRID/等高线 → T/G/C/D 显隐 → 3D 漫游。", "blue")
    m.h2("11.1 后续 GUI 升级")
    m.para("当前已交付 2D/3D 浏览、任意多边形节点级 GRID 裁剪/掩膜、同 CRS 错位 GRID 显式重采样、匹配现状/设计 GRID 的解析挖填方与差值图、可取消并行地形专题，以及 TIN/CDT/GRID 局部坡面、剖面、水平基准土方、格式转换、GDAL 交换及约束编辑。后续重点是坐标系显式重投影、GRID 映射文件与流式瓦片输出、局部 CDT 更新、网格单元解析边界裁剪、误差控制、可配置图层样式、GPU 分块 LOD 与贴地碰撞。")
